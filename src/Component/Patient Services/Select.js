...
export default memo(function Search() {
  ...
  const handleSearch = () => {
    if (selectedVal.current === "") {
      ...
      const sData = Data.filter(item => item.PatientName.toLowerCase().includes(selectedVal.current.toLowerCase()) || item.Doctor.toLowerCase().includes(selectedVal.current.toLowerCase()) || item.DoctServices.toLowerCase().includes(selectedVal.current.toLowerCase()) || item.EntryDate.toLowerCase().includes(selectedVal.current.toLowerCase()));
      setData(sData);
    }
  };
